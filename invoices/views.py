...
def invoices(request):
    ...
    if sort == 'invoice':
        client_invoices = sorted(client_invoices, key=lambda x: x["invoice"].id, reverse=True)
    elif sort == 'person':
        client_invoices = sorted(client_invoices, key=lambda x: x["invoice"].person.firstname)
    elif sort == 'assignee':
        client_invoices = sorted(client_invoices, key=lambda x: x["invoice"].modified_by.first_name)
    elif sort == 'registered':
        client_invoices = sorted(client_invoices, key=lambda x: x["invoice"].created_at, reverse=True)
    elif sort == 'deadline':
        client_invoices = sorted(client_invoices, key=lambda x: x["invoice"].deadline, reverse=True)
    elif sort == 'status':
        client_invoices = sorted(client_invoices, key=lambda x: x["invoice"].status.id)
    elif sort == 'value':
        client_invoices = sorted(client_invoices, key=lambda x: x["value"], reverse=True)
    elif sort == 'payed':
        client_invoices = sorted(client_invoices, key=lambda x: x["payed"])
    elif sort == 'update':
        client_invoices = sorted(client_invoices, key=lambda x: x["invoice"].modified_at, reverse=True)
    else:
        client_invoices = sorted(client_invoices, key=lambda x: x["invoice"].created_at, reverse=True)
